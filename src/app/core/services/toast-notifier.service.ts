...
export class ToastNotifierService {
  ...
  /** Карта «событие домена → стиль тоста». Чего нет в карте — не озвучивается */
  ...
  constructor() {
    // Не реплеим события, накопленные до старта нотификера
    // (иначе после перезагрузки страницы всплыл бы старый тост)
    this.lastProcessedEventId = this.eventLog.events()[0]?.id ?? null;

    effect(() => {
      const events = this.eventLog.events();

      // Собираем все события новее последнего обработанного:
      // за один синхронный сценарий их может быть несколько
      ...
      // fresh пришёл newest-first — озвучиваем в хронологическом порядке
